...
[商业版激活绑定教程](https://snail007.github.io/goproxy/page/free_vs_commercial/)
...
`--bind-listen`参数，就可以开启客户端用`入口IP`连接过来的，就用`入口IP`作为`出口IP`访问目标网站的功能。
如果绑定了不正确的IP会导致代理不能工作，此时代理会尝试不绑定IP去访问目标，同时日志会提示。  
...
本机有IP：`5.5.5.5`，开一个http代理端口`6666`，凡是通过`5.5.5.5:6666`端口进来的，都使用`5.5.5.5`作为出口IP。
...
可以使用参数`--bind-ip`参数，格式：`IP:端口`，比如： `1.1.1.1:8080`，`[2000:0:0:0:0:0:0:1]:8080`

比如本机有IP`5.5.5.5`,`6.6.6.6`，如下命令：

`proxy http -t tcp -p :8888,7777 --bind-ip 5.5.5.5`
